...
public class NumUtils {
	...
	public static int numQuarters(int cents) {
		...
		// note: cents/QUARTER_VALUE also works, however it is good to get into 
		// the habit of using the classname qualifier though (clearer to read, and 
		// know where to look up this value if you want to check later)

		return (cents / NumUtils.QUARTER_VALUE);		}
	...
	public static int wrapAngle(int degrees) {
		...
		// recall: -value % divisor (will give a negative remainder)

		return ( degrees % NumUtils.DEGREES_IN_CIRCLE );  	}
	...
	public static double avg(int a, int b) {
		...
		// NOTE: converting a or b to double will ensure a+b is promoted to 
		// double, and does not overflow before dividing by 2.  Also once numerator
		// is a double, the division will also be double (not integer division)
	...
	public static double windChill(double airTemp, double windSpeed) {
		...
		// not checking explicitly for use of constants, but this should be habit
		final double OFFSET = 13.12;
		final double AIR_FACTOR = 0.6215;
		final double WIND_FACTOR = 11.37;
		final double WIND_EXP = Math.pow(windSpeed, 0.16);
		final double COMBINED_FACTOR = 0.3965;
		
		double wChill = OFFSET  + AIR_FACTOR*airTemp 
								- WIND_FACTOR*WIND_EXP 
								+ COMBINED_FACTOR*airTemp*WIND_EXP;
